function path = educationalFemBemTikhonovPath(model, target, alphas, options)
%EDUCATIONALFEMBEMTIKHONOVPATH Trace-fit regularization path for FEM/BEM education.
%
...
% over educationalFemBemTraceLeastSquares, not a black-box optimizer.
...
if isempty(alphaValues)
    error("educationalFemBemTikhonovPath:alphas", ...
...
for k = 1:numel(alphaValues)
    fit = educationalFemBemTraceLeastSquares(model, target, ...
...
tol = 1e-10;
path = struct();
path.kind = "educational_fem_bem_tikhonov_path";
