function result = boxConstrainedLeastSquares(A, b, lower, upper, options)
%boxConstrainedLeastSquares Readable projected-gradient gate.
%
...
n = size(A, 2);
if size(A, 1) ~= numel(b)
    error("boxConstrainedLeastSquares:dimension", ...
        "A row count must match the number of entries in b.");
end
if numel(lower) ~= n || numel(upper) ~= n
    error("boxConstrainedLeastSquares:bounds", ...
...
if numel(rawInitial) ~= n
    error("boxConstrainedLeastSquares:initial", ...
...
if stepSize <= 0
    error("boxConstrainedLeastSquares:step", ...
...
result = struct();
result.kind = "box_constrained_least_squares";
