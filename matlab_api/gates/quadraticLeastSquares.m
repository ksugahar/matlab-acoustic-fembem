function result = quadraticLeastSquares(A, b, options)
%quadraticLeastSquares Readable quadratic least-squares gate.
%
...
n = size(A, 2);
if size(A, 1) ~= numel(b)
    error("quadraticLeastSquares:dimension", ...
...
if numel(x0) ~= n
    error("quadraticLeastSquares:initial", ...
...
result = struct();
result.kind = "quadratic_least_squares";
