function result = femBemTraceLeastSquares(model, target, options)
%FEMBEMTRACELEASTSQUARES Fit boundary data through a FEM/BEM trace.
%
% This small teaching gate connects the readable FEM/BEM model to
% optimization:
%
...
nFem = size(T, 2);
if size(T, 1) ~= numel(g)
    error("femBemTraceLeastSquares:target", ...
...
if numel(u0) ~= nFem
    error("femBemTraceLeastSquares:initial", ...
...
result = struct();
result.kind = "fem_bem_trace_least_squares";
