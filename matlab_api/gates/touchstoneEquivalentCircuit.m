...
result = primary;
result.kind = "touchstone_equivalent_circuit";
...
if ~isnan(options.ComparisonZ0)
    if options.ComparisonZ0 <= 0
        error("touchstoneEquivalentCircuit:comparisonZ0", ...
...
dY = det(I + S);
if abs(dY) <= tol
    error("touchstoneEquivalentCircuit:singularY", ...
        "I+S is singular; short-circuit admittance matrix is undefined.");
end

dZ = det(I - S);
if abs(dZ) <= tol
    error("touchstoneEquivalentCircuit:singularZ", ...
