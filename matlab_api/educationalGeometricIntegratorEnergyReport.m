function report = educationalGeometricIntegratorEnergyReport(stepSize, steps, omega)
%EDUCATIONALGEOMETRICINTEGRATORENERGYREPORT Harmonic-oscillator energy gate.
%
...
if ~(isscalar(stepSize) && isfinite(stepSize) && stepSize > 0)
    error("educationalGeometricIntegratorEnergyReport:stepSize", ...
        "stepSize must be a positive scalar.");
end
if ~(isscalar(steps) && isfinite(steps) && steps == round(steps) && steps > 0)
    error("educationalGeometricIntegratorEnergyReport:steps", ...
        "steps must be a positive integer.");
end
if ~(isscalar(omega) && isfinite(omega) && omega > 0)
    error("educationalGeometricIntegratorEnergyReport:omega", ...
...
report = struct();
report.kind = "educational_geometric_integrator_energy_report";
