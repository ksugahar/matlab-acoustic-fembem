function report = lowFrequencyHelmholtzTeachingReport(targetPoint, sourcePoint, options)
%LOWFREQUENCYHELMHOLTZTEACHINGREPORT Explain the low-frequency BEM split.
%
...
r = norm(delta);
if r <= 0
    error("lowFrequencyHelmholtzTeachingReport:distance", ...
        "targetPoint and sourcePoint must be distinct.");
end

k = options.Wavenumber;
kernel = lowFrequencyStableHelmholtzKernel(targetPoint, sourcePoint, ...
    "Wavenumber", k, ...
