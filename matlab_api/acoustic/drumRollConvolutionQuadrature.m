...
%DRUMROLLCONVOLUTIONQUADRATURE Two-location alternating strikes (a drum roll) by CQ.
%
%   result = drumRollConvolutionQuadrature() radiates a drum roll -- the same
%   body struck ALTERNATELY at TWO different spots -- with the Lubich CQ
%   time-domain BEM.  Each beat is a localized Ricker "tap": spot A fires on
%   beats 1,3,5,..., spot B on beats 2,4,6,...  The only thing that changes vs a
%   single strike is BoundaryTimeData (N x nBoundary); the CQ solver carries the
%   alternating taps to the listeners.  This showcases the solver's arbitrary
%   space-time boundary excitation.
%
%   Two default listeners sit on the +x and -x sides so the roll is directional:
...
% --- two strike spots: the surface nodes furthest along +x and -x ------------- %
[~, iA] = max(X(:,1));  spotA = X(iA, :);         % +x pole
[~, iB] = min(X(:,1));  spotB = X(iB, :);         % -x pole
wA = exp(-sum((X - spotA).^2, 2) / (2*options.StrikeWidthSpace^2));   % spatial bumps
...
% --- default listeners on the +x (A) and -x (B) sides ------------------------- %
obs = options.ObservationPoints;
if isempty(obs)
    obs = [ options.ListenerRadius 0 0; -options.ListenerRadius 0 0];
end

% --- radiate the drum roll with the CQ solver -------------------------------- %
