function result = educationalTouchstoneDesignFrequencyGrid(frequencies, designFrequency, options)
%EDUCATIONALTOUCHSTONEDESIGNFREQUENCYGRID Check sweep rows around a design frequency.
%
...
if isempty(frequencies)
    error("educationalTouchstoneDesignFrequencyGrid:empty", ...
        "frequencies must contain at least one row");
end
if ~isvector(frequencies)
    error("educationalTouchstoneDesignFrequencyGrid:vector", ...
...
if any(diff(frequencyHz) <= 0)
    error("educationalTouchstoneDesignFrequencyGrid:monotonic", ...
...
result = struct();
result.kind = "educational_touchstone_design_frequency_grid";
...
switch lower(strtrim(unit))
    ...
    case "ghz"
        scale = 1e9;
    otherwise
        error("educationalTouchstoneDesignFrequencyGrid:frequencyUnit", ...
