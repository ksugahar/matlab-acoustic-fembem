function result = educationalFarfieldLobeNotebookHandoff(thetaValues, phiValues, lobeRow, options)
%EDUCATIONALFARFIELDLOBENOTEBOOKHANDOFF Check one far-field lobe row.
%
...
metadata = educationalFarfieldPatternMetadata(thetaValues, phiValues, ...
...
gainGate = educationalFarfieldGainDirectivityEfficiency(gainDbi, directivityDbi, ...
...
result = struct();
result.kind = "educational_farfield_lobe_notebook_handoff";
...
result.notes = [
    "run this after educationalFarfieldPatternMetadata and before plotting or ranking lobes"
