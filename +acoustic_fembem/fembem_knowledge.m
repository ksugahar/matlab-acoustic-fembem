...
s = strjoin([
    "# Curved (isoparametric) panel geometry -- SHIPPED"
    ""
    "The curve-only-geometry design is now implemented as a curved-panel BEM"
    "lane.  It keeps first-order UNKNOWNS (P1 boundary density) and curves only"
    "the GEOMETRY -- a quadratic (6-node) isoparametric panel -- so it is the"
    "readable 'curved P1' (superparametric) element, exactly the design that was"
    "previously only sketched here.  This removes the O(h^2) straight-panel"
    "faceting error that caps the flat BEM (see ACOUSTIC / the"
    "testHelmholtzScattering note that its analytic deviation is"
    "'faceted-geometry dominated')."
    ""
    "Shipped API (matlab_api/bem, matlab_api/mesh):"
    "  - CurvedPanelQuadrature(surface, order, projection): the small adapter"
    "    that owns the 6 geometry nodes per triangle, the quadratic map, curved"
    "    Jacobian weights (w * 0.5 * |x_u x x_v|), and outward normals.  It is"
    "    the curved counterpart of SurfaceQuadrature."
    "  - curvedSingleLayerDirichletSolve / curvedCollocationSingleLayer: the P1"
    "    nodal-collocation single layer on curved panels (Duffy vertex-singular"
    "    quadrature on the curved panel; far panels by plain curved Gauss)."
    ""
    "One knob = one A/B.  Projection = @(X) X leaves the edge nodes at the"
    "straight midpoints, the quadratic map degenerates to the affine one, and"
    "the curved quadrature reproduces SurfaceQuadrature to 2e-16 -- so the flat"
    "lane is literally the identity-projection special case.  A surface"
    "projection (CurvedPanelQuadrature.sphereProjection(R)) curves the panels;"
    "nothing else changes, so the comparison isolates geometry."
    ""
    "Measured (unit sphere, R2026a; validation_test/testCurvedPanelSphereConvergence"
    "+ committed curvedPanelSphereConvergence.json):"
    "  - Laplace capacitance -> 4*pi*R: flat 1.3e-2/5.4e-3 -> curved 1.7e-4/2.9e-5"
    "    (coarse/fine); 74x/186x closer."
    "  - Sound-soft plane-wave scattering vs the partial-wave series:"
    "    k=0.5 flat 1.4e-2/6.1e-3 -> curved 6.0e-4/1.5e-4 (24x/40x);"
    "    k=2.0 flat 3.6e-2/1.4e-2 -> curved 5.0e-3/1.2e-3 (7x/12x)."
    "  Curving helps most at low k (geometry-dominated) and still 7-12x at k=2."
    ""
    "Curve order is the lever, not fes order.  With the SOLUTION fixed at P1,"
    "sweeping the CURVE order 1->2->3 (testCurvedPanelCurveOrderSweep) shows the"
    "two error channels: Laplace capacitance keeps improving (1.3e-2 -> 1.7e-4 ->"
    "4.7e-5, coarse) because geometry stays the bottleneck, while k=2 scattering"
    "jumps 1->2 (7x) then PLATEAUS 2->3 (5.0e-3 -> 4.7e-3) because the P1 density"
    "error is now the floor.  Lesson: geometry error and field error are separate"
    "channels; curve order is the binding lever WHILE geometry is the bottleneck,"
    "and fes order takes over once it is resolved -- so on a curved boundary raise"
    "the curve order first."
    ""
    "Scope caveat -- self-generate, do NOT parse a Netgen curved .vol.  This lane"
    "builds the curved panels by projecting each straight edge midpoint onto an"
    "analytic surface (the sphere), NOT by reading a high-order .vol.  A Netgen"
    "curved .vol stores its curving as a 'curvedelements' section that is"
    "Netgen's INTERNAL integrated-Legendre / Jacobi(alpha,beta) coefficient basis"
    "(curvedelems.cpp), NOT node coordinates -- reproducing that basis in MATLAB"
    "is fragile (a wrong normalization/orientation makes Load succeed with a"
    "silently wrong geometry).  So the readable route is: self-generate"
    "isoparametric panels for an analytic surface, or (for a general mesh) let"
    "NGSolve export convention-free node COORDINATES and feed those to the"
    "CurvedPanelQuadrature adapter."
    ""
    "Optional netgen path -- SHIPPED.  tools/export_curved_boundary_nodes.py"
    "(NGSolve) evaluates each boundary triangle's curved geometry map at the"
    "Lagrange nodes and writes the physical COORDINATES to a JSON companion; the"
    "saved .vol stays P1, so MATLAB never touches the coefficient basis."
    "curvedQuadratureFromNetgen(surface, json) matches the exported triangles"
    "onto the SurfaceMesh by corner coordinates (raises on a miss) and builds a"
    "curve-order-2 CurvedPanelQuadrature; a GeomNodes option threads the explicit"
    "nodes through curvedSingleLayerDirichletSolve.  A real netgen OCC sphere"
    "(fixtures/curved_panels/) reaches the SAME accuracy as the analytic-"
    "projection lane -- capacitance 3.4e-4, scatter k2 6.5e-3, within ~1.6x of"
    "analytic and ~40x/6x below flat (validation_test/testCurvedPanelNetgenVol)."
    "So 'netgen curved mesh -> MATLAB high accuracy' works for GENERAL geometry,"
