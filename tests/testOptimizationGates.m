function tests = testOptimizationGates
%TESTOPTIMIZATIONGATES Tests for readable optimization gates.
...
result = quadraticLeastSquares(A, b, "Initial", x0);

verifyEqual(testCase, result.kind, "quadratic_least_squares");
...
result = quadraticLeastSquares(A, b, ...
    "Initial", x0, ...
...
verifyError(testCase, @() quadraticLeastSquares(A, b, "Initial", 0), ...
    "quadraticLeastSquares:initial");
end
...
result = boxConstrainedLeastSquares(A, b, lower, upper, ...
    "Initial", [0; 0], ...
    "StepSize", 1.0, ...
    "MaxIterations", 5);

verifyEqual(testCase, result.kind, "box_constrained_least_squares");
...
verifyError(testCase, @() boxConstrainedLeastSquares(A, b, 2, 1), ...
    "boxConstrainedLeastSquares:bounds");
end
...
result = femBemTraceLeastSquares(model, target);

verifyEqual(testCase, result.kind, "fem_bem_trace_least_squares");
...
regularization = femBemTikhonovPath(model, target, [0; 1e-3; 1e-1; 1], ...
    "Initial", ones(5, 1), ...
    "GradientTolerance", 1e-6);

verifyEqual(testCase, regularization.kind, "fem_bem_tikhonov_path");
...
corner = femBemLcurveCorner(model, target, [1e-4; 1e-3; 1e-2; 1e-1; 1], ...
    "Initial", ones(5, 1), ...
    "GradientTolerance", 1e-6);

verifyEqual(testCase, corner.kind, "fem_bem_lcurve_corner");
...
regularization = femBemTikhonovPath(model, target, alphas, ...
    "Initial", ones(5, 1), ...
    "GradientTolerance", 1e-6);
noiseNorm = regularization.weightedTraceResiduals(4);

choice = femBemMorozovDiscrepancy(model, target, alphas, noiseNorm, ...
    "Initial", ones(5, 1), ...
    "GradientTolerance", 1e-6);

verifyEqual(testCase, choice.kind, "fem_bem_morozov_discrepancy");
...
result = touchstonePortMetadata(["P1" "P2"], ...
    "RequiredPorts", ["P1" "P2"], ...
    "PortOrder", ["P1" "P2"], ...
    "NetworkParameter", "S", ...
    "DataFormat", "MA", ...
...
verifyEqual(testCase, result.kind, "touchstone_port_metadata");
...
swapped = touchstonePortMetadata(["P2" "P1"], ...
    "PortOrder", ["P1" "P2"], ...
    "NetworkParameter", "S", ...
    "DataFormat", "MA", ...
    "FrequencyUnit", "GHz", ...
...
missingZ0 = touchstonePortMetadata(["P1" "P2"], ...
    "PortOrder", ["P1" "P2"], ...
    "NetworkParameter", "S", ...
    "DataFormat", "MA", ...
    "FrequencyUnit", "GHz");
...
wrongFormat = touchstonePortMetadata(["P1" "P2"], ...
    "PortOrder", ["P1" "P2"], ...
    "NetworkParameter", "S", ...
    "DataFormat", "DB", ...
    "ExpectedDataFormat", "MA", ...
...
result = farfieldPatternMetadata([0 90 180], [0 90], ...
    "FrequencyHz", 2.45e9, ...
...
verifyEqual(testCase, result.kind, "farfield_pattern_metadata");
...
wrongBasis = farfieldPatternMetadata([0 90 180], [0 90], ...
    "FrequencyHz", 2.45e9, ...
    "AngleUnit", "deg", ...
    "CoordinateSystem", "spherical", ...
...
missingNormalization = farfieldPatternMetadata([0 90 180], [0 90], ...
    "FrequencyHz", 2.45e9, ...
    "AngleUnit", "deg", ...
    "CoordinateSystem", "spherical", ...
    "PolarizationBasis", "theta_phi", ...
...
narrowTheta = farfieldPatternMetadata([0 60 120], [0 90], ...
    "FrequencyHz", 2.45e9, ...
    "AngleUnit", "deg", ...
    "CoordinateSystem", "spherical", ...
    "PolarizationBasis", "theta_phi", ...
...
result = farfieldGainDirectivityEfficiency(gainDbi, directivityDbi, ...
    "RadiatedPowerW", 6.5, ...
    "AcceptedPowerW", 10.0, ...
    "Normalization", "accepted_power");

verifyEqual(testCase, result.kind, "farfield_gain_directivity_efficiency");
...
tooHighGain = farfieldGainDirectivityEfficiency(directivityDbi + 0.2, directivityDbi, ...
    "RadiatedPowerW", 6.5, ...
    "AcceptedPowerW", 10.0, ...
    "Normalization", "accepted_power");
...
missingNormalization = farfieldGainDirectivityEfficiency(gainDbi, directivityDbi, ...
    "RadiatedPowerW", 6.5, ...
    "AcceptedPowerW", 10.0);
verifyEqual(testCase, missingNormalization.status, "needs_attention");
verifyFalse(testCase, missingNormalization.checks.normalizationRecorded);

inconsistentPower = farfieldGainDirectivityEfficiency(gainDbi, directivityDbi, ...
    "RadiatedPowerW", 5.0, ...
    "AcceptedPowerW", 10.0, ...
    "Normalization", "accepted_power");
...
result = farfieldLobeNotebookHandoff([0 90 180], [0 90], lobe, ...
    "FrequencyHz", 2.45e9, ...
    "ExpectedFrequencyHz", 2.45e9, ...
    "AngleUnit", "deg", ...
    "CoordinateSystem", "spherical", ...
...
verifyEqual(testCase, result.kind, "farfield_lobe_notebook_handoff");
...
missing = farfieldLobeNotebookHandoff([0 90 180], [0 90], missingLobe, ...
    "FrequencyHz", 2.45e9, ...
    "AngleUnit", "deg", ...
    "CoordinateSystem", "spherical", ...
    "PolarizationBasis", "theta_phi", ...
...
wrongCut = farfieldLobeNotebookHandoff([0 90 180], [0 90], wrongPhi, ...
    "FrequencyHz", 2.45e9, ...
    "AngleUnit", "deg", ...
    "CoordinateSystem", "spherical", ...
    "PolarizationBasis", "theta_phi", ...
...
badGain = farfieldLobeNotebookHandoff([0 90 180], [0 90], tooHigh, ...
    "FrequencyHz", 2.45e9, ...
    "AngleUnit", "deg", ...
    "CoordinateSystem", "spherical", ...
    "PolarizationBasis", "theta_phi", ...
...
result = touchstoneEquivalentCircuit(0, 0, ...
    "S12", 0, ...
    "S22", 0, ...
    "Z0", 50, ...
    "ComparisonZ0", 75);

verifyEqual(testCase, result.kind, "touchstone_equivalent_circuit");
...
active = touchstoneEquivalentCircuit(0, 1.1, "S12", 1.1, "S22", 0);
verifyEqual(testCase, active.status, "needs_attention");
verifyFalse(testCase, active.checks.sparameterPassivityOk);

verifyError(testCase, @() touchstoneEquivalentCircuit(0, 0, "ComparisonZ0", -75), ...
    "touchstoneEquivalentCircuit:comparisonZ0");
end


function testTouchstonePortMatchKeepsS11SeparateFromInsertionLoss(testCase)
result = touchstonePortMatch(1/3, ...
    "ReturnLossMinDb", 9.0, ...
    "VswrMax", 2.1);

verifyEqual(testCase, result.kind, "touchstone_port_match");
...
active = touchstonePortMatch(1.02);
verifyEqual(testCase, active.status, "needs_attention");
...
result = touchstoneDesignFrequencyGrid([0.95 0.99 1.01 1.05], 1.0, ...
    "FrequencyUnit", "GHz", ...
    "MaxRelativeSpacing", 0.03);

verifyEqual(testCase, result.kind, "touchstone_design_frequency_grid");
...
exact = touchstoneDesignFrequencyGrid([0.9; 1.0; 1.1], 1.0);
...
coarse = touchstoneDesignFrequencyGrid([0.8 1.2], 1.0, ...
    "FrequencyUnit", "GHz", ...
...
outside = touchstoneDesignFrequencyGrid([0.8 0.9], 1.0, ...
    "FrequencyUnit", "GHz");
verifyEqual(testCase, outside.status, "needs_attention");
verifyFalse(testCase, outside.checks.designFrequencyBracketed);

verifyError(testCase, @() touchstoneDesignFrequencyGrid([0.9 0.9 1.0], 1.0), ...
    "touchstoneDesignFrequencyGrid:monotonic");
end
...
result = touchstoneSolverReadyPreflight(0.05, s21, ...
    "S12", s21, ...
    "S22", 0.05, ...
    "Frequency", 1.0, ...
    "FrequencyUnit", "GHz", ...
...
verifyEqual(testCase, result.kind, "touchstone_solver_ready_preflight");
...
active = touchstoneSolverReadyPreflight(0.05, 1.2, ...
    "S12", 1.2, ...
    "S22", 0.05, ...
    "Frequency", 1.0);
verifyEqual(testCase, active.status, "needs_attention");
...
missingFrequency = touchstoneSolverReadyPreflight(0.05, s21, ...
    "S12", s21, ...
    "S22", 0.05);
...
path = string(fullfile(tempdir, "optimizationGates_" + char(java.util.UUID.randomUUID()) + ".vol"));
fid = fopen(path, "w");
